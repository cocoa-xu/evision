...
ifeq ($(MIX_TARGET), ios)
	...
	CMAKE_EVISION_OPTIONS += -DCMAKE_OSX_SYSROOT=visionos -DSYSTEM=visionOS -DARCH=$(VISIONOS_ARCH)
endif
ifdef TARGET_GCC_FLAGS
...
$(HEADERS_TXT): $(CONFIGURATION_PRIVATE_HPP)
	@ if [ "$(EVISION_PREFER_PRECOMPILED)" != "true" ] && [ "$(EVISION_COMPILE_WITH_REBAR)" != "true" ]; then \
		...
		if [ "$(MIX_TARGET)" = "ios" ]; then \
			...
			xcodebuild BITCODE_GENERATION_MODE=bitcode VISIONOS_DEPLOYMENT_TARGET=$(VISIONOS_DEPLOYMENT_TARGET) ARCHS=$(VISIONOS_ARCH) -sdk visionos -configuration $(CMAKE_BUILD_TYPE) -parallelizeTargets -jobs $(DEFAULT_JOBS) -target ALL_BUILD build ; \
			cmake -DBUILD_TYPE=$(CMAKE_BUILD_TYPE) -P cmake_install.cmake ; \
		else \
