#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Orchestrator for the evision binding generator.

`Pipeline` walks parsed OpenCV headers and drives the C++/Elixir/Erlang
emitters in order: parse -> resolve inheritance bases -> emit class glue
-> emit namespace functions -> emit enums/constants -> finalize per-module
outputs -> write to disk.

This was originally `BeamWrapperGenerator` inside gen2.py. The body has
not been functionally split yet — see step 8 follow-up.
"""
...
from fixes import (
    evision_elixir_fixes,
    evision_erlang_fixes,
    evision_elixir_module_fixes,
               ...
               struct_name = 'Evision'
                ...
                if is_struct(original_name):
                    ...
                    if not any([x[0].islower() or x[0] == '_' for x in original_name.split('.')]):
                        ...
                        the_name = NAMESPACE_TO_ELIXIR_MODULE.get(original_name, None)
