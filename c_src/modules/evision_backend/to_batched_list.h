...
// @evision c: mat_to_batched_list,evision_cv_mat_to_batched_list,1
// @evision nif: def mat_to_batched_list(_opts \\ []), do: :erlang.nif_error("Mat::to_batched_list not loaded")
static ERL_NIF_TERM evision_cv_mat_to_batched_list(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    using namespace cv;
    ...
    if (evision_to_safe(env, evision_get_kw(env, erl_terms, "img"), img, ArgInfo("l", 0)) &&
        ...
        if (leftover != "repeat" && leftover != "discard") {
            return evision::nif::error(env, "to_batched_list failed: invalid option value for leftover. Valid values are :repeat and :discard");
        }
        ...
        if (mat_num_elem != as_shape_num_elem) {
            return evision::nif::error(env, "to_batched_list failed: cannot treated matrix as the request shape");
        }
