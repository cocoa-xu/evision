defmodule Evision.Wx do
  ...
  def init({window_name, window}) do
    :wx.batch(fn ->
      ...
      {frame, %{window_name: window_name, window: window, frame: frame, panel: panel, canvas: canvas, bitmap: bitmap, image: nil}}
    end)
  end

  @impl true
  def handle_event({:wx, _, _, _, {:wxSize, :size, {w, h}, _}}, state=%{canvas: canvas, bitmap: prev, image: image}) do
    if w > 0 and h > 0 do
  ...
  @impl true
  def handle_sync_event({:wx, _, _, _, {:wxPaint, :paint}}, _obj, %{canvas: canvas, bitmap: bitmap, image: image}) do
    draw_canvas(image, canvas, bitmap)
    :ok
  end
  ...
  defp draw_canvas(image, canvas, bitmap) when is_struct(image, Evision.Mat) do
    ...
    {image_h, image_w} = case Evision.Mat.shape(image) do
      {h, w} ->
        {h, w}
    ...
    with true <- image_h > 0 and image_w > 0 and canvas_h > 0 and canvas_w > 0,
      ...
      :wxDC.blit(canvasDC, {0, 0}, {:wxBitmap.getWidth(bitmap), :wxBitmap.getHeight(bitmap)}, memoryDC, {0, 0})
