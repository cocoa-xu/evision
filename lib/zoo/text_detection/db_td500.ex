defmodule Evision.Zoo.TextDetection.DB_TD500 do
  ...
  def to_quoted(attrs) do
    ...
    [
      quote do
        model = Evision.Zoo.TextDetection.DB_TD500.init(unquote(model), unquote(opts))
      end,
      quote do
        ...
        |> Kino.listen(fn %{data: %{image: image}} ->
          ...
          {detections, confidences} = Evision.Zoo.TextDetection.DB_TD500.infer(model, image_)

          image_ = Evision.cvtColor(image_, Evision.cv_COLOR_RGB2BGR())
          vis_img = Evision.Zoo.TextDetection.DB_TD500.visualize(image_, detections, confidences)
